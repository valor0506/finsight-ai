...
from report_builder.docx_builder import build_commodity_report
...
def generate_report(self, report_id, asset_type, asset_symbol, analysis_type, user_id):
    logger.info(f"Starting: {asset_symbol} ({asset_type})")
    ...
    try:
        ...
        if asset_type == "commodity":
            ...
            report_bytes = build_commodity_report(asset_symbol, market_data, analysis["text"], macro_data)
            # report_bytes = build_commodity_report(asset_symbol, market_data, analysis, macro_data)
            ...
            if analysis.get("error"):
                raise ValueError(f"Analysis failed: {analysis['error']}")
            report_bytes = build_equity_report(asset_symbol, market_data, analysis["text"], macro_data)
            # if analysis.get("error"):
            #     raise ValueError(f"Analysis failed: {analysis['error']}")
            # report_bytes = build_commodity_report(asset_symbol, market_data, analysis, macro_data)
        ...
        filename = f"{asset_symbol}_{report_id[:8]}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.docx"
        file_url = _upload_to_supabase(report_bytes, filename)
        _update_report_status(report_id, "completed", file_url=file_url)
        logger.info(f"Completed: {file_url}")
